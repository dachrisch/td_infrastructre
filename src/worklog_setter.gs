function configure_selections(selections, issue_key, billable = false, hour_factor = 1) {
  ...
  const configured_events = selections.map(selection => {
    let calendar_event = getActiveCalendar().getEventById(selection.calendar_id)
    if (!calendar_event) {
      ...
      calendar_event = getActiveCalendar().getEvents(entry.start_date, new Date(moment(entry.start_date).add(entry.duration, 'seconds'))).filter(event => event.getId() == selection.calendar_id)[0]
    }
    if (!calendar_event) {
