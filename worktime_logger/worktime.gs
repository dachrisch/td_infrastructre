function exportWorktime() {
  ...
  s.getSheet('overview').clear()
    ...
    .on('E2:E').when().aboveEqual(0).background('lightgreen').build().when().below(0).background('orange').build().and()
    .on('G4').setHeader('overtime').and().on('H4').setFormula('=sum(E2:E)').when().aboveEqual(0).background('lightgreen').build().when().below(0).background('orange').build()

  let index = 2
  for(var momentInMonth = momentFrom.clone();momentInMonth.isBefore(momentTo); momentInMonth.add(1, 'month')) {
    s.getSheet('overview').on(`A${index}`).setValues(momentInMonth.format('DD.MM.YYYY')).and()
